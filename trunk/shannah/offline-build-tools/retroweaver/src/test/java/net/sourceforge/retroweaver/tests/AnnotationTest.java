...
class Child3 extends Ancestor {
	...
	public static void main(String args[]) throws Exception {
		Method m = Child3.class.getMethod("foo");

		Annotation[] annotations = m.getAnnotations();
		System.out.println("length: " + annotations.length);
		System.out.println(annotations[0].toString());

		Deprecated a1 = m.getAnnotation(Deprecated.class);
		System.out.println("is Deprecated null: " + (a1 == null));
	}
...
public class AnnotationTest extends AbstractTest {
	...
	public void testJavaLangAnnotations() throws Exception {
		Method m = Child3.class.getMethod("foo");

		Annotation[] annotations = m.getAnnotations();
		assertEquals("length", 1, annotations.length);

		Deprecated a1 = m.getAnnotation(Deprecated.class);
		assertNotNull("Deprecated", a1);

		Override a2 = m.getAnnotation(Override.class);
		assertNull("Override", a2);
		
		SuppressWarnings a3 = m.getAnnotation(SuppressWarnings.class);
		assertNull("SuppressWarnings", a3);
	}
	...
	public @interface MainAnnotation {
		NestedAnnotation[] array();
	}

	@MainAnnotation(array = {@NestedAnnotation(name="n1", description="desc", id = "id1"), @NestedAnnotation(name="n2", id="id2")})
	public void testNestedAnnotations() throws NoSuchMethodException {
		Method m = this.getClass().getDeclaredMethod("testNestedAnnotations");

		assertTrue(m.isAnnotationPresent(MainAnnotation.class));

		NestedAnnotation[] annotationArray = m.getAnnotation(MainAnnotation.class).array();
		
		NestedAnnotation a = annotationArray[0];
		assertEquals("valid description 0", "desc", a.description());
		assertEquals("valid id 0", "id1", a.id());
		assertEquals("valid name 0", "n1", a.name());

		a = annotationArray[1];
		assertEquals("valid description 1", "default description", a.description());
		assertEquals("valid id 1", "id2", a.id());
		assertEquals("valid name 1", "n2", a.name());
	}
